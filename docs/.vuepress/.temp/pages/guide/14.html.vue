<template><h1 id="_14-编译器和解释器-v8是如何执行一段javascript代码的" tabindex="-1"><a class="header-anchor" href="#_14-编译器和解释器-v8是如何执行一段javascript代码的" aria-hidden="true">#</a> 14 | 编译器和解释器：V8是如何执行一段JavaScript代码的？</h1>
...
<p>要深入理解 V8 的工作原理，你需要搞清楚一些概念和原理，比如接下来我们要详细讲解的**编译器（Compiler）、解释器（Interpreter）、抽象语法树（AST）、字节码（Bytecode）、即时编译器（JIT）**等概念，都是你需要重点关注的。</p>
<h2 id="编译器和解释器" tabindex="-1"><a class="header-anchor" href="#编译器和解释器" aria-hidden="true">#</a> 编译器和解释器</h2>
...
<p><img src="https://static001.geekbang.org/resource/image/4e/81/4e196603ecb78188e99e963e251b9781.png?wh=1142*510" alt="编译器和解释器“翻译”代码"></p>
...
<h2 id="v8-是如何执行一段-javascript-代码的" tabindex="-1"><a class="header-anchor" href="#v8-是如何执行一段-javascript-代码的" aria-hidden="true">#</a> V8 是如何执行一段 JavaScript 代码的</h2>
<p>通过上面的介绍，相信你已经了解编译器和解释器了。那接下来，我们就重点分析下 V8 是如何执行一段 JavaScript 代码的。你可以先来“一览全局”，参考下图：</p>
<p><img src="https://static001.geekbang.org/resource/image/1a/ae/1af282bdc4036096c03074da53eb84ae.png?wh=1142*522" alt="V8 执行一段代码流程图"></p>
...
<h3 id="_1-生成抽象语法树-ast-和执行上下文" tabindex="-1"><a class="header-anchor" href="#_1-生成抽象语法树-ast-和执行上下文" aria-hidden="true">#</a> 1. 生成抽象语法树（AST）和执行上下文</h3>
<p>将源代码转换为<strong>抽象语法树</strong>，并生成<strong>执行上下文</strong>，而执行上下文我们在前面的文章中已经介绍过很多了，主要是代码在执行过程中的环境信息。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br></div></div><p>这段代码经过javascript-ast站点处理后，生成的 AST 结构如下：</p>
<p><img src="https://static001.geekbang.org/resource/image/73/36/7320526ef14d974be8393effcf25b436.png?wh=1142*1644" alt="抽象语法树（AST）结构"></p>
...
<p><strong>第一阶段是分词（tokenize），又称为词法分析</strong>，其作用是将一行行的源码拆解成一个个 token。所谓 <strong>token</strong>，指的是语法上不可能再分的、最小的单个字符或字符串。你可以参考下图来更好地理解什么 token。</p>
<p><img src="https://static001.geekbang.org/resource/image/83/f5/838028071f63a132cc8b27b23960e5f5.png?wh=1142*505" alt="分解 token 示意图"></p>
...
<h3 id="_2-生成字节码" tabindex="-1"><a class="header-anchor" href="#_2-生成字节码" aria-hidden="true">#</a> 2. 生成字节码</h3>
...
<p><img src="https://static001.geekbang.org/resource/image/87/ff/87d1ab147d1dc4b78488e2443d58a3ff.png?wh=1142*314" alt="字节码和机器码占用空间对比"></p>
<p>从图中可以看出，机器码所占用的空间远远超过了字节码，所以使用字节码可以减少系统的内存使用。</p>
<h3 id="_3-执行代码" tabindex="-1"><a class="header-anchor" href="#_3-执行代码" aria-hidden="true">#</a> 3. 执行代码</h3>
...
<p><img src="https://static001.geekbang.org/resource/image/66/8a/662413313149f66fe0880113cb6ab98a.png?wh=766*912" alt="即时编译（JIT）技术"></p>
<h2 id="javascript-的性能优化" tabindex="-1"><a class="header-anchor" href="#javascript-的性能优化" aria-hidden="true">#</a> JavaScript 的性能优化</h2>
...
<ol>
<li>
<p>提升单次脚本的执行速度，避免 JavaScript 的长任务霸占主线程，这样可以使得页面快速响应交互；</p>
</li>
<li>
<p>避免大的内联脚本，因为在解析 HTML 的过程中，解析和编译也会占用主线程；</p>
</li>
<li>
<p>减少 JavaScript 文件的容量，因为更小的文件会提升下载速度，并且占用更低的内存。</p>
</li>
</ol>
...
<ul>
<li>
<p>首先我们介绍了编译器和解释器的区别。</p>
</li>
<li>
<p>紧接着又详细分析了 V8 是如何执行一段 JavaScript 代码的：V8 依据 JavaScript 代码生成 AST 和执行上下文，再基于 AST 生成字节码，然后通过解释器执行字节码，通过编译器来优化编译字节码。</p>
</li>
<li>
<p>基于字节码和编译器，我们又介绍了 JIT 技术。</p>
</li>
<li>
<p>最后我们延伸说明了下优化 JavaScript 性能的一些策略。</p>
</li>
</ul>
...
<p>最后留给你个思考题：你是怎么理解“V8 执行时间越久，执行效率越高”这个性质的？</p>
