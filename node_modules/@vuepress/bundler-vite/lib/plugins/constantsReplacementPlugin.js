...
exports.constantsReplacementPlugin = void 0;
 ...
 */
const constantsReplacementPlugin = (app) => {
    const pagesDirPrefix = `${app.dir.temp('pages')}/`;
    let constantsRegexp;
    const replaceConstants = (code, breaker) => code.replace(constantsRegexp, (constant) => `${constant[0]}${breaker}${constant.slice(1)}`);
    return {
        ...
        configResolved(resolvedConfig) {
            ...
            constantsRegexp = new RegExp(`\\b(${constants
        ...
        transform(code, id) {
            ...
            if (match === null)
                return;
            return (code.substring(0, match.index) +
                replaceConstants(match[0], vueTemplateBreaker) +
                code.substring(match.index + match[0].length));
        },
    };
};
exports.constantsReplacementPlugin = constantsReplacementPlugin;
