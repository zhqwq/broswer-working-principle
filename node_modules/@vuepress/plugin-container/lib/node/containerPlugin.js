...
validate, marker, render, }) => {
    ...
    if (!type) {
        utils_1.logger.warn(`[${plugin.name}] ${utils_1.chalk.magenta('type')} option is required`);
        return plugin;
    }
        ...
        render = (tokens, index, opts, env) => {
            ...
            if (token.nesting === 1) {
                ...
                if (!info && locales) {
                    ...
                    const relativePath = (0, shared_1.ensureLeadingSlash)(filePathRelative ?? '');
                    const localePath = (0, shared_1.resolveLocalePath)(locales, relativePath);
